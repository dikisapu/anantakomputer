<?php
	 ...
	 */ 
	 class view {
		...
		function jual_range($tgl_awal, $tgl_akhir){
    $sql = "SELECT nota.*, barang.id_barang, barang.nama_barang, barang.harga_beli, member.id_member, member.nm_member
            FROM nota
            LEFT JOIN barang ON barang.id_barang = nota.id_barang
            LEFT JOIN member ON member.id_member = nota.id_member
            WHERE nota.tanggal_input BETWEEN ? AND ?
    ...
    return $hasil;
		 ...
		 function nama_jual($nama_barang) {
        $sql = "SELECT nota.*, barang.id_barang, barang.nama_barang, barang.harga_beli, member.id_member, member.nm_member
                FROM nota
                LEFT JOIN barang ON barang.id_barang = nota.id_barang
                LEFT JOIN member ON member.id_member = nota.id_member
        ...
        $stmt = $this->db->prepare($sql);
        $stmt->execute(["%$nama_barang%"]); // Menggunakan LIKE untuk mencocokkan nama barang
        $hasil = $stmt->fetchAll();
